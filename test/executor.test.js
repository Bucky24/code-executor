...
const { number, callFunction, conditional, comparison, variable, createFunction, assign, math, returnFn, cls } = require("../src/helpers");
...
describe('Executor', () => {
    describe('execute', () => {
        ...
        it('should call a global context function', async () => {
            const code = { type: STRUCTURE_TYPE.FUNCTION_CALL, name: variable('foo'), arguments: [] };
            let called = false;
        ...
        it('should fail to call a function that is not a function', async () => {
            const code = [
                ...
                { type: STRUCTURE_TYPE.FUNCTION_CALL, name: variable('foo'), arguments: [] },
            ];
            const executor = new Executor(code);
        ...
        it('should pass arguments to the function', async () => {
            const code = { type: STRUCTURE_TYPE.FUNCTION_CALL, name: variable('foo'), arguments: [ number(1) ] };
            let args = null;
        ...
        it('should handle return value of raw function', async() => {
            const code = assign('blar', callFunction(variable('foo'), []));
            let calledContext = null;
            const context = Executor.createContext({}, { foo: (arg, context) => {
                calledContext = context;
                return {
        ...
        describe('conditionals', () => {
            it('should execute the children if the condition is true', async () => {
                const code =[
                    { type: STRUCTURE_TYPE.CONDITIONAL, condition: { type: STRUCTURE_TYPE.COMPARISON, left: number(1), right: number(1), operator: COMPARISON_OPERATOR.EQUAL }, children: [{ type: STRUCTURE_TYPE.FUNCTION_CALL, name: variable('foo'), arguments: [] }] },
                ];
                let called = false;
                const context = Executor.createContext({}, { foo: () => {
                    called = true;
            ...
            it('should not execute the children if the condition is false', async () => {
                const code = [
                    { type: STRUCTURE_TYPE.CONDITIONAL, condition: { type: STRUCTURE_TYPE.COMPARISON, left: number(2), right: number(1), operator: COMPARISON_OPERATOR.EQUAL }, children: [{ type: STRUCTURE_TYPE.FUNCTION_CALL, name: variable('foo'), arguments: [] }] },
                ];
                let called = false;
                const context = Executor.createContext({}, { foo: () => {
                    called = true;
                ...
                const code = [
                    {
                        ...
                        children: [
                            conditional(comparison(number(1), number(1), COMPARISON_OPERATOR.EQUAL), [callFunction(variable('foo'), [])]),
                            conditional(comparison(number(2), number(1), COMPARISON_OPERATOR.EQUAL), [callFunction(variable('bar'), [])]),
                        ],
                    },
                ];
                let calledFoo = false;
                ...
                const code = [
                    {
                        type: STRUCTURE_TYPE.CONDITIONAL_GROUP,
                        children: [
                            conditional(comparison(number(2), number(1), COMPARISON_OPERATOR.EQUAL), [callFunction(variable('foo'), [])]),
                            conditional(comparison(number(1), number(1), COMPARISON_OPERATOR.EQUAL), [callFunction(variable('bar'), [])]),
                        ],
                    },
                ];
                let calledFoo = false;
                ...
                const code = [
                    {
                        ...
                        finally: callFunction(variable('bar'), []),
                    },
                ];
                let calledFoo = false;
                let calledBar = false;
                ...
                const code = [
                    {
                        type: STRUCTURE_TYPE.CONDITIONAL_GROUP,
                        children: [
                            conditional(comparison(number(1), number(1), COMPARISON_OPERATOR.EQUAL), [returnFn(number(5)),callFunction(variable('foo'), [])]),
                            conditional(comparison(number(1), number(1), COMPARISON_OPERATOR.EQUAL), [callFunction(variable('bar'), [])]),
                        ],
                        finally: callFunction(variable('bar'), []),
                    },
        ...
        describe('internal function calls', () => {
            ...
            it('should handle the return value of an internal function', async () => {
                const code = [
                    ...
                    assign('myvar', callFunction(variable('foo'), [])),
                ];
                const executor = new Executor(code);
        ...
        describe('paths', () => {
            ...
            it('should call a function as part of a path', async () => {
                const code = [
                    assign('foo', number(3)),
                    assign('myvar', { type: STRUCTURE_TYPE.OBJECT, properties: { foo: { type: STRUCTURE_TYPE.OBJECT, properties: { bar: createFunction(null, [], [assign('foo', number(4))]) } } }}),
                    callFunction({ type: STRUCTURE_TYPE.PATH, path: ['foo', 'bar'], left: variable('myvar')}, []),
                ];
                const executor = new Executor(code);

                await executor.execute();

                expect(executor.getTopLevelContext().variables.foo).toEqual({ type: VALUE_TYPE.NUMBER, value: 4});
            });
        });
        ...
        describe('classes', () => {
            it.only('should allow calling a function on a class', async () => {
                const code = [
                    ...
                    callFunction({ type: STRUCTURE_TYPE.PATH, left: variable('foo'), path: ['bar']}, []),
                ];
